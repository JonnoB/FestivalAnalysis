...
corpus<- list.files(pattern = "Glastonbury") %>% map(~LexisNexisSource(.x, encoding = "UTF-8") %>%
...
source21 <-source2[[1]]
source22 <-source2[[2]]
z=c(source21,source22, recursive = T)

corpus <- Corpus(source2[[2]], readerControl = list(language = NA))

test <- do.call(function(...) c(..., recursive = TRUE), source)
...
test <- ProcessArticles("Glastonbury")
...
ProcessArticles<- function(FestivalName){
  ...
  source<- list.files(pattern = FestivalName) %>% map(~LexisNexisSource(.x, encoding = "UTF-8"))
 
  #collapses the list of corpora into a single corpus
  source <- do.call(function(...) c(..., recursive = TRUE), source)
